...
export const metadata: Metadata = { title: 'Set up your gift' }
...
export default async function NewGiftPage({ searchParams }: Props) {
  ...
  if (!key || key.status !== 'redeemed') redirect('/activate')
    ...
    <main className="min-h-dvh bg-parchment">
      <div className="container-fluid relative z-10 py-12">
        <div className="mb-10 text-center">
          ...
          <h1 className="font-display text-4xl font-medium text-ink">Let&apos;s build your gift.</h1>
          <p className="mt-2 text-sm text-ink-muted">A few details to get started. You can change everything later.</p>
        </div>
        <GiftSetupForm activationKeyId={keyId} experienceType={type || 'digital_gift'} />
      </div>
    </main>
  )
}
